  ...
  if (!robot_->is_connected()) {
    if (!warned) {
      RCLCPP_WARN_THROTTLE(
          ...
          "Robot not connected. Skipping info update. Check that the robot is connected to the computer and that permissions are set correctly.");
      warned = true;
    }
    return;
  }
...
void RobotDriver::publish_robot_status() {
  ...
  if (!robot_->is_connected()) {
    if (!warned) {
      RCLCPP_WARN_THROTTLE(
          get_logger(), *get_clock(), 5000,
          "Robot not connected. Skipping status update. Check that the robot is connected to the computer and that permissions are set correctly.");
      warned = true;
    }
    return;
  }
...
void RobotDriver::update_odom() {
  ...
  if (!robot_->is_connected()) {
    if (!warned) {
      RCLCPP_WARN_THROTTLE(
          get_logger(), *get_clock(), 5000,
          "Robot not connected. Skipping odometry update. Check that the robot is connected to the computer and that permissions are set correctly.");
      warned = true;
    }
    return;
  }
...
void RobotDriver::velocity_event_callback(
  ...
  if (!robot_->is_connected()) {
    if (!warned) {
      RCLCPP_WARN_THROTTLE(
          get_logger(), *get_clock(), 5000,
          "Robot not connected. Skipping velocity command. Check that the robot is connected to the computer and that permissions are set correctly.");
      warned = true;
    }
    return;
  }
